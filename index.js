...
if (process.env.NODE_ENV != "test") {
  (async () => {
    ...
    await listBranches(userId, "GitTest");
    await createRepo(userId, "new-repo");
    await createIssue(userId, "new-repo", "new issue", "New Issue Body");
    await enableWikiSupport(userId, "new-repo");
  })();
}
...
async function enableWikiSupport(owner, repo) {
  ...
  return new Promise(function (resolve, reject) {
    request(options, function (error, response, body) {
      resolve(JSON.parse(body));
      console.log("Wiki support has been enabled.");
    });
  });
}
